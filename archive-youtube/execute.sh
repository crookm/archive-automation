...
youtube-dl --add-metadata --no-overwrites --restrict-filenames --output "%(playlist)s/%(playlist_index)s-%(title)s-%(id)s.%(ext)s" --ignore-errors --no-warnings --quiet --download-archive $ARCHIVE_FILE --batch-file $PLAYLIST_DIR
